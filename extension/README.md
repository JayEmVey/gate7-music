...
4. Pin the extension and click its toolbar icon. The dashboard loads the public playlist catalog and shows a sign-in prompt on its first load. Click **Connect Spotify** to open the authorization window, then sign in as the allowlisted manager. Cache status and sync controls require successful authentication. Interactive authorization starts only from this explicit click, following Chrome's identity API guidance.
5. Click **Sync all playlists** once to create the initial baseline, then enable **Auto-detected synchronization** if desired.

To distribute a ZIP, run `npm run extension:package`. This creates `build/gate7-playlist-sync-1.0.1.zip`. Extract it before choosing **Load unpacked**. No extension build step or third-party browser scripts are needed.

If upgrading an existing unpacked installation, replace its files (or use this repository's `extension` folder), click **Reload** on its card at `chrome://extensions`, and reopen the dashboard.

### When Chrome says the authorization page could not be loaded

This error comes from Chrome's OAuth window before Spotify returns an authorization code. It does not by itself identify the cause. The dashboard expands **Connection setup** after a failed sign-in and shows the actual client ID returned by the deployed Worker and the actual redirect URL generated by Chrome.

1. Match that client ID to the app in the Spotify developer dashboard and register the exact displayed callback, including the `/spotify` path and no extra trailing slash.
2. Check that `https://accounts.spotify.com` loads in Chrome; browser policy, filtering extensions, proxies or network restrictions can also prevent the OAuth window from loading.
3. Reload this extension and click **Connect Spotify** again. Cancelling sign-in leaves the public playlist catalog visible, with **Sign in to check** instead of an unverified cache status. Sync stays disabled.

The extension cannot inspect or change the redirect allowlist inside your Spotify developer account. Seeing Spotify's login page load is not proof that the callback is registered; a complete sign-in is still required to verify it.
